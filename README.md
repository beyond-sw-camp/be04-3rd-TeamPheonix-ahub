...
| 이름 | 내용 |
| ----- | ----------|
|    백동현   | 벌써 세번째 프로젝트까지 끝났다니 시간이 되게 빠르다고 느껴졌습니다. 백엔드는 다뤄봤던 적이 있어서 조금은 익숙했지만 프론트는 처음이라 html/css 부터 vue까지 모든게 하나같이 다 어렵고 낯설었습니다. 그래서 굉장히 막막했지만 구글링과 팀원들의 도움을 받아 완성할 수 있었습니다. 프로젝트의 마무리가 다가올수록 욕심이 나서 이런저런 기능을 더 추가하고 싶었지만 시간이 없어서 기능들을 더 추가를 못한것이 굉장히 아쉬웠고 그래서 프로젝트가 끝나더라도 추가로 더 고도화를 진행해야겠다고 생각했습니다. 이번 프로젝트까지 하며 디비, 백, 프론트 모두 다 다뤄보게되었는데 이 경험들을 통해서 최종프로젝트는 좀 더 완벽하게 만들 수 있을것같다고 느꼈습니다. |
| 손세림 | 이번 프론트엔드 프로젝트를 통해 ahub 커뮤니티 사이트를 개발하는 과정에서 많은 것을 배우고 경험했습니다. 백엔드 서버와 프론트 화면을 연동하는 과정에서 axios를 활용했으나, CORS 오류로 인해 예상보다 시간이 지체되었습니다. 이로 인해 다양한 기능을 구현하지 못한 점이 아쉬움이 남기도 하지만, 이를 통해 CORS 문제를 해결하는 다양한 방법에 대해 배울 수 있었습니다. 추가로, 이번 프로젝트 기간 내에 구현하지 못한 아이디 중복 검사, 토큰을 활용한 회원 권한 부여 등의 기능을 지속적으로 개발해 나갈 예정입니다. 또한, 정리되지 않은 코드를 지속적으로 리팩토링하여 코드의 가독성과 효율성을 향상시키고 프로젝트를 발전 시켜나갈 예정입니다. 이번 프로젝트를 통해 많은 것을 배우고 성장한 소중한 경험이었습니다. 앞으로도 계속해서 발전하고 더 나은 프로젝트를 위해 노력하겠습니다. |
| 송동준 | 이번 프로젝트는 HTML, CSS, Vue, JavaScript를 사용하여 백앤드로만 구현했던 저번 프로젝트를 프론트엔드를 이용하여 실제로 동작하는 과정을 화면단에서 볼 수 있게 하는 프로젝트였습니다. 먼저 Visily를 사용하여 웹페이지를 대략적으로 스케치하는 과정을 거쳤고 그 이후 Visily에서 스케치한 내용을 HTML을 틀로 잡어 CSS를 통해 디자인하고, Fetch를 통한 API 서버 통신, JavaScript를 사용해 이벤트 처리 비동기 통신 그리고 Vue를 사용한 Route 기술을 사용하면서 실제로 화면상에 구현해 보았습니다. 이번 프로젝트를 통해 저희가 스케치한 내용이 실제로 작동하는 과정을 두 눈으로 확인하면서 성취감을 느끼고 흥미로웠습니다. 비록 유독 짧았던 프로젝트 시간으로 막막하기도 했고 처음 구상했던 몇몇 정렬 기능이나 검색 기능 등을 구현하지 못하였지만 팀원들과 다 함께 고민하고 모르는 부분을 팀원들한테 배우면서 최선의 성과를 낼 수 있었습니다. 구현하지 못한 기능들도 팀원들과 함께 한다면 끝까지 해낼 수 있다는 자신감이 생겼습니다.|
|    양지혜    | 처음으로 프론트엔드 화면 단을 구성해 보니 보람찬 한편 제 미숙함에 아쉬움이 많았습니다. 또한 axios와 router를 이용해 정보 및 화면을 연결하는 작업이 저에게는 꽤 어렵게 다가왔습니다. 팀원들의 도움이 없었더라면 결코 기간 내에 작업을 완료하지 못했을 겁니다. 프로젝트 마감 기한은 다가오는데 점점 추가할 페이지나 기능들이 눈에 띄어 조금씩 수정하게 되었습니다. 그럼에도 친절하게 업데이트해 주신 취합자에게 감사함을 전하고 싶습니다. 팀원들이 백엔드, 프론트엔드 작업을 통해 다양한 기능을 구현하는 것을 옆에서 몸소 지켜보면서 많은 점을 배웠습니다. 조금씩 개발자로서 성장하는 제 모습을 상상하며 다음 프로젝트에는 좀 더 성숙한 태도로 임하겠습니다. |
| 정태원 | 프론트엔드는 백엔드에 비해 쉽다는 말을 많이 들어서 쉽게 생각했지만, 막상 만들고 보니 굉장히 아쉬운 점이 많이 남은 프로젝트였습니다. 맡은 파트에 대해서 grid 나 flex를 사용해서 조금 더 깔끔한 화면을 구현할 수 있을 것 같고, 이외에도 반응형 객체를 사용하여, 조건에 따라서 화면 출력이 달라지게 만드는 기능들이 더 추가 되어야 할 것 같습니다. 이 외에도 v-for 나 v-html 등 vue 관련 기능에 대한 공부나, 자바 스크립트에 대해서 전반적으로 더 공부가 필요하겠다는 생각이 강하게 들었습니다. 가장 안타까웠던 점은 css에 신경을 많이 쓰느라 동적인 화면을 구성하는 것이 좀 부족했다는 점 입니다. 공부를 하면서 프로젝트를 진행할 때 외견보다는 기능에 더 집중하는 것을 꼭 명심하고 다음 프로젝트를 진행해야 할 듯 합니다. 아직 여러모로 많이 부족한 프로젝트지만, 꾸준히 개발을 하면서 올 여름 쯤에는 실제로 서비스를 할 수 있을 정도로 고도화가 진행되었으면 좋겠습니다. 이상입니다. |
